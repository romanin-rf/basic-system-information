...
class root_operation:
	try:
		...
		attributes = settings.std.cfgd["root"]["attributes"]
		with open(settings.ConfigFileName, "w") as config_data_file:
			json.dump(settings.std.cfgd, config_data_file)
...
IndicatorAverageLoadLabel = tk.Label(root, text = "Загрузка...", font = 'helvetica 10 bold', bg = "black", fg = "white", width = 10)
#		Счётчики
...
TextSWAPLoadLabel = tk.Label(root, text = "Подкачка (Занято): Загрузка...", font = 'helvetica 10 bold')
TextSWAPFreeLabel = tk.Label(root, text = "Подкачка (Свободно): Загрузка...", font = 'helvetica 10 bold')
...
def UpdateSWAPLoadLabel():
	while function_operation.UpdateSWAP:
		try:
			...
			TextSWAPLoadLabel["text"] = "Подкачка (Занято): {0} МБ/{1} МБ ({2} %)".format(used_mem_mb, total_mem_mb, used_mem_present)
		except:
			TextSWAPLoadLabel["text"] = "Подкачка (Занято): Ошибка..."
...
def UpdateSWAPFreeLabel():
	while function_operation.UpdateSWAP:
		try:
			...
			if type(free_mem_mb) == float:
				free_mem_mb = rounding_float(free_mem_mb)
			TextSWAPFreeLabel["text"] = "Подкачка (Свободно): {0} МБ/{1} МБ ({2} %)".format(free_mem_mb, total_mem_mb, free_mem_present)
		except:
			TextSWAPFreeLabel["text"] = "Подкачка (Свободно): Ошибка..."
		time.sleep(timeouts.UpdateSWAP)
